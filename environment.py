...
class ShoverWorldEnv(gym.Env):
    ...
    def _validate_boxes_not_on_edges(cls, grid: np.ndarray) -> None:
        ...
        for r in range(n_rows):
            for c in range(n_cols):
                if cls._is_box_value(grid[r, c]) and (
                    r == 0 or c == 0 or r == n_rows - 1 or c == n_cols - 1
                ):
                    raise ValueError("Box placed on map edge is not allowed.")
    ...
    def _update_perfect_squares(self, initial: bool) -> List[Tuple[int, int, int]]:
        """
        Detect current perfect squares, update their ages, and automatically
        dissolve any whose age >= perf_sq_initial_age.

        Returns:
            List of (n, top, left) keys for squares that were dissolved this step.
        """
    ...
    def _apply_barrier_maker(
        ...
        """
        Convert the oldest perfect square (n >= 2) into barriers.

        Returns:
            (ok, stamina_gain, reward_gain, square_key)
            where square_key is (n, top, left) of the affected square, or None.
        """
    ...
    def _apply_hellify(self) -> Tuple[bool, int, Optional[Tuple[int, int, int]]]:
        """
        Hellify the oldest perfect square with n > 2.

        Returns:
            (ok, destroyed, square_key)
            where destroyed is the number of boxes removed and square_key is
            (n, top, left) of the affected square, or None.
        """
        sq = self._get_oldest_square(lambda n: n > 2)
        if sq is None:
            return False, 0, None
